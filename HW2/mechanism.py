...
class Mechanism:
    ...
    def __init__(self, dataset, specs):
        ...
        for col in list(domain_info):
            if col in self.specs:
                try:
                    if isinstance(domain_info[col], list) and len(domain_info[col]) > 0:
                        last_val = domain_info[col][-1]
                        if isinstance(last_val, str) and last_val.isdigit():
                            last_val = int(last_val)
                        if isinstance(last_val, (int, float)) and last_val < self.specs[col]['maxval']:
                            print('Codebook inconsistent for', col)
                            del domain_info[col]
                except (TypeError, ValueError) as e:
                    print(f"Warning: Cannot compare values for {col}: {e}")
    ...
    def load_data(self, path=None):
        ...
        for col in self.domain_info:
            if col in df.columns and isinstance(self.domain_info[col], list):
