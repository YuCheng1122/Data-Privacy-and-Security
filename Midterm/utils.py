...
def differentially_private_pca(data, n_components=60, epsilon=1.0, delta=1e-5):
    """
    Differentially private PCA implementation
    """
    # Flatten data if not already flattened
    data_np = data.cpu().numpy().reshape(len(data), -1)
    
    # Normalize data (make each sample unit norm)
    ...
    # Compute sensitivity and add noise
    sensitivity = 2  # For unit vectors
    ...
    # Return the components as a tensor
